...
$dbname = "bdfilms";
  ...
  function getRentals($conn){
    try {
      $stmt = $conn->prepare('SELECT f.*, m.nom As member_name, m.email As member_email, l.quantity FROM films f JOIN locations l ON f.id=l.film_id JOIN membres m ON m.id=l.membre_id');
      $stmt->execute();
  ...
  function register($name,$email,$password,$conn){
    try {
      ...
      die("Error While registering the user: " . $e->getMessage());
    }
  }
  ...
  function newFilm($data,$conn){
    try {
      ...
      die("Error While adding to Films: " . $e->getMessage());
    }
  }
  ...
  function updateFilm($id,$data,$conn,$toDelete){
    try {
      ...
      die("Error While updating the Film: " . $e->getMessage());
    }
  }
  function deleteMovie($film_id,$conn){
    try {
      ...
      die("Error While deleting a movie: " . $e->getMessage());
    }
  }
  ...
  function deleteMember($member_id,$conn){
    try {
      ...
      return  "<script type='text/javascript'>toastr.success('Le Membre a été Supprimé avec Succès !')</script>";
    } catch(PDOException $e) {
      die("Error While deleting a movie: " . $e->getMessage());
    }
  }
   ...
   function deleteCategory($category_id,$conn){
    try {
      ...
      return  "<script type='text/javascript'>toastr.success('La Catégorie a été Supprimée avec Succès !')</script>";
    } catch(PDOException $e) {
      die("Error While deleting a movie: " . $e->getMessage());
    }
  }
  ...
  function getCategoryById($id,$conn){
    try {
      ...
      die("Error While getting a Category By Id: " . $e->getMessage());
    }
  ...
  function newCategory($titre,$conn){
    try {
      ...
      die("Error While adding to categories: " . $e->getMessage());
    }
  }
  ...
  function updateCategory($id,$titre,$conn){
    try {
      ...
      die("Error While updating the Category: " . $e->getMessage());
    }
  }
